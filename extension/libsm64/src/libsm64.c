...
SM64_LIB_FN void sm64_get_limb_transform( int32_t marioId, int32_t limbId, struct SM64LimbTransform *outTransform )
{
    if( marioId >= s_mario_instance_pool.size || s_mario_instance_pool.objects[marioId] == NULL )
    {
        return;
    }
    if (limbId < 0 || limbId >= 21)
    {
        return;
    }
    ...
    outTransform->position[0] = globalState->mgLimbMatrices[limbId][3][0];
    outTransform->position[1] = globalState->mgLimbMatrices[limbId][3][1];
    outTransform->position[2] = globalState->mgLimbMatrices[limbId][3][2];

    float m[3][3];
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            m[i][j] = globalState->mgLimbMatrices[limbId][j][i];
        }
    }
    ...
    outTransform->rotation[0] = qx;
    outTransform->rotation[1] = qy;
    outTransform->rotation[2] = qz;
    outTransform->rotation[3] = qw;
}
