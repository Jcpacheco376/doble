...
int gCurrentAnimatedPartIndex = 0;
...
static void geo_process_animated_part(struct GraphNodeAnimatedPart *node) {
    ...
    gMatStackIndex++;
    if (gCurrentAnimatedPartIndex < 21) {
        mtxf_copy(g_state->mgLimbMatrices[gCurrentAnimatedPartIndex], gMatStack[gMatStackIndex]);
        gCurrentAnimatedPartIndex++;
    }
